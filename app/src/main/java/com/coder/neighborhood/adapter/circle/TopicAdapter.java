...
import com.coder.neighborhood.activity.circle.TopicDetailActivity;
...
public class TopicAdapter extends RvAdapter<TopicBean>{
    ...
    @Override
    protected RvViewHolder<TopicBean> getViewHolder(int viewType, View view) {
        return new EventViewHolder(view);
    }

    class EventViewHolder extends RvViewHolder<TopicBean>{
        ...
        @Override
        public void onBindData(int position, TopicBean bean) {
            ...
            tvContent.setText(bean.getTopicsInfo());
            if (bean.getTopicsDate()!=0){
                tvSubScribe.setText(TimeUtils.milliseconds2String(bean.getTopicsDate(),new SimpleDateFormat("yyyy.MM.dd")));
            }

            itemView.setOnClickListener(v -> TopicDetailActivity.start(getContext(),bean.getTopicsId()));
        }
    }
}
